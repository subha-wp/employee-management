...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  const [employee, setEmployee] = useState<Omit<Employee, "password"> | null>(null)
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      const authenticatedEmployee = await dataManager.validateEmployee(email, password)
      ...
      localStorage.setItem("employee_data", JSON.stringify(authenticatedEmployee))
  ...
  const logout = async () => {
    if (employee) {
      try {
        const activeTimeEntry = await dataManager.getActiveTimeEntry(employee.id)
        if (activeTimeEntry) {
          const now = new Date()
